...
model_name = "LSTM_network_diffs_additional_features_200epochs"
number_points = [5,6,7,8,9,10]
prediction_position = 0
features_x = [0,1,2,3,5]
features_y = [0,1,2,3,5]
f_diff = [0,1,3] 
...
for nop in number_points:
    ...
    with torch.no_grad():
        ...
        if(len(f_diff)>0):
            xs_test_descaled_dediff, ys_test_descaled_dediff = from_diff(originals_testing, xs_test_descaled, ys_test_descaled, [0,1,3])


        model_predictions = np.zeros((len(xs_test), num_predictions, 3))
        for i in range(num_predictions):
            ...
            if(len(f_diff)>0):
                xs_test_descaled_dediff, out_descaled_dediff = from_diff(originals_testing, xs_test_descaled, out_descaled, [0,1,3])
            predictions = list(zip(xs_test_descaled_dediff, out_descaled_dediff, ys_test_descaled_dediff[:,prediction_position+i,:])) 
            model_predictions[:,i,:] = np.squeeze(out_descaled_dediff[:,:,[0,1,3]])
            total_loss = 0
