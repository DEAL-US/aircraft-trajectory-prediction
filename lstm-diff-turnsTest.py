...
model_name = "LSTM_network_diffs_trueturnTest_200epochs"
...
for nop in number_points:
    ...
    # Removal of samples with -1 (no value) for any of the features. May need to replace -1 with another value to avoid cases with features being valued -1
    keep_positions = []
    for pos, (x, y) in enumerate(zip(xs, ys)):
        if -1 in x[:,features_x] or -1 in y[prediction_position:(prediction_position+num_predictions),features_y]:
    ...
    xs_turns = np.copy(xs[keep_positions])
    ys_turns = np.copy(ys[keep_positions])
    ids_turns = np.copy(ids[keep_positions])
    
    if(len(f_diff)>0):
        originals_turns = originals[keep_positions]

    rnd = np.random.default_rng(1337)
    positions = rnd.permutation(len(xs))
    rnd = np.random.default_rng(1337)
    positions_turns = rnd.permutation(len(xs_turns))
    num_training = int(len(xs)*training_fraction)
    pos_training = positions[:num_training]
    num_training_turns = int(len(xs_turns)*training_fraction)
    pos_testing = positions_turns[num_training_turns:]

    xs_training = xs[pos_training]
    ys_training = ys[pos_training]
    ids_training = ids[pos_training]
    xs_test = xs_turns[pos_testing]
    ys_test = ys_turns[pos_testing]
    ids_testing = ids_turns[pos_testing]

    if(len(f_diff)>0):
        originals_training = originals[pos_training]
        originals_testing = originals_turns[pos_testing]
    ...
    with torch.no_grad():
        ...
        for i in range(num_predictions):
            ...
            # TODO Case when there are no diffs, in which we directly take the descaled variables as the dediffs ones
