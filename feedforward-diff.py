...
model_name = "feed-forward_network_diffs"
...
for nop in number_points:
    ...
    # dataset_test = CustomDataset(xs_test, ys_test)
    loader_training = dataloader.DataLoader(dataset_training, batch_size)

    # model = architectures.AttentionNetwork(len(features_x), seq_size)
    ...
    # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [3,7], 0.1, verbose=True)
    ...
    with torch.no_grad():
        ...
        for i in range(num_predictions):
            out = np.expand_dims(model(torch.Tensor(xs_test).to(device).float()).cpu().detach().numpy(), 1)
            xs_test_descaled = scaler.scale_back(xs_test)
