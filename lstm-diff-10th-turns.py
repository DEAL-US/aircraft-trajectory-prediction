...
model_name = "LSTM_network_diffs_10th_prediction_turns"
number_points = [5,6,7,8,9,10]
prediction_position = 9
features_x = [0,1,3]
features_y = [0,1,3]
f_diff = [0,1,3] 
...
for nop in number_points:
    ...
    # Removal of samples with -1 (no value) for any of the features.
    keep_positions = []
    for pos, (x, y) in enumerate(zip(xs, ys)):
        if -1 in x[:,features_x] or -1 in y[:,features_y]:
    ...
    with open(f'./training-data-turns/{nop}', 'rb') as f:
    ...
    ys = ys[:,0:prediction_position+1,:].cumsum(1)[:,prediction_position,:]
